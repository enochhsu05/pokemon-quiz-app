...
games = {'red-blue': 1, 'yellow': 2, 'gold-silver': 3, 'crystal': 4, 'ruby-sapphire': 5, 'emerald': 6, 'firered-leafgreen': 7, 'diamond-pearl': 8, 'platinum': 9, 'heartgold-soulsilver': 10, 'black-white': 11, 'colosseum': 12, 'xd': 13, 'black-2-white-2': 14, 'x-y': 15, 'omega-ruby-alpha-sapphire': 16, 'sun-moon': 17, 'ultra-sun-ultra-moon': 18, 'lets-go-pikachu-lets-go-eevee': 19, 'sword-shield': 20, 'brilliant-diamond-and-shining-pearl': 21, 'scarlet-violet': 22}
generations = [0, 151, 251, 386, 493, 649, 721, 809, 905, 1025]
...
def get_species_by_name(name: str):
    url = f"{BASE_URL}/pokemon-species/{name.lower()}"
    try:
        response = requests.get(url)
        return response.json()
    except Exception:
        return to_json(get_pokemon_by_name(name)['species'])
...
def guess_pokemon_by_type(order=False):
    ...
    if secondary_type is not None:
        ...
        if order:
            return get_typing(input_pokemon) == [primary_type, secondary_type]
        return all(type in [primary_type, secondary_type] for type in get_typing(input_pokemon))
    else:
        input_pokemon = input(f'Name a {primary_type} type pokemon: ')
        return get_typing(input_pokemon) == [primary_type]


def get_typing(name: str):
    mon = get_pokemon_by_name(name)
...
def generate_random_pokemon(start_gen = 1, end_gen = 9):
    start_index = generations[start_gen - 1] + 1
    end_index = generations[end_gen]
    return get_pokemon_by_number(random.randint(start_index, end_index))
...
def get_level_up_moveset(mon, game_order=22, ordered=True):  # does not include level-up moves that are also learnable by tm??
    ...
    for move in mon['moves']:
        ...
        if order <= game_order:
            for i in range(len(version_details)):
                game_number = games[version_details[i]['version_group']['name']]
...
def get_generation(name):
    dex_num = get_pokemon_by_name(name)['id']
    i = 1
    while dex_num > generations[i]:
        i += 1
    return i


def get_base_stats(name):
    return [stat['base_stat'] for stat in get_pokemon_by_name(name)['stats']]


def get_color(name):
    return get_species_by_name(name)['color']['name']


def get_flavor_text(name):
    entries = get_species_by_name(name)['flavor_text_entries']
    i = 0
    while entries[i]['language']['name'] != 'en':
        i += 1
    entry = get_species_by_name(name)['flavor_text_entries'][i]['flavor_text'].lower()
    entry = entry.replace(name, 'this pokemon')
    entry = entry[:1].upper() + entry[1:]
    for i in range(len(entry)):
        if entry[i] == '.' and i < len(entry) - 1:
            entry = entry[:i+2] + entry[i+2].upper() + entry[i+3:]
    return entry


def get_abilities(name: str) -> list[str]:
    abilities = [ability['ability']['name'] for ability in get_pokemon_by_name(name)['abilities']]
